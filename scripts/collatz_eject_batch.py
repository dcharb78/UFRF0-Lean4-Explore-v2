...
def word(n: int) -> str:
    if n not in NUMBER_WORDS:
        if 0 < n < 100:
            ...
            tens_word = NUMBER_WORDS.get(tens * 10)
            ones_word = NUMBER_WORDS.get(ones)
            if tens_word is not None and ones_word is not None:
                return tens_word if ones == 0 else f"{tens_word}_{ones_word}"
