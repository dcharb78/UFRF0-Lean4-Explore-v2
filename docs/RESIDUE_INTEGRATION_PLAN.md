...
- Current Phase 3 status for a broader contour layer:
  ...
  alpha/CODATA-gap transport now reaches `d27`, the exposed `AlphaRunning`
  gap/residual/error package now reaches `d27`, and the prediction package
  also reaches `d27`. If further exposed tightening is wanted, the next
  proof-heavy blocker is stronger static alpha/CODATA-gap transport beyond
  `d27` rather than additional contour infrastructure or more
  prediction-only packaging.
